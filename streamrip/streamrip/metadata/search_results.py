...
class ArtistSummary(Summary):
    ...
    def from_item(cls, item: dict):
        ...
        num_albums = item.get("nb_album") or item.get("albums_count") or "Unknown"
...
class TrackSummary(Summary):
    ...
    def from_item(cls, item: dict):
        ...
        artist = item.get("artist", {}).get("name") or "Unknown"
        date_released = item.get("release_date") or "Unknown"
...
class AlbumSummary(Summary):
    ...
    def from_item(cls, item: dict):
        ...
        artist = item.get("artist", {}).get("name") or "Unknown"
        num_tracks = item.get("nb_tracks", 0)
        date_released = item.get("release_date") or "Unknown"
...
class LabelSummary(Summary):
    ...
    @classmethod
    def from_item(cls, item: dict):
        return cls(str(item["id"]), item["name"])
...
class PlaylistSummary(Summary):
    ...
    def from_item(cls, item: dict):
        ...
        creator = item.get("user", {}).get("name") or "Unknown"
        num_tracks = item.get("nb_tracks") or len(item.get("tracks", [])) or -1
