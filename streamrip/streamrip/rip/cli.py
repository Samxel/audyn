...
@click.option(
    "-q",
    "--quality",
    help="The maximum quality allowed to download (0=MP3_128, 1=MP3_320, 2=FLAC)",
...
def rip(
    ...
    """Streamrip: Deezer music downloader."""
    global logger
    logging.basicConfig(
        level="INFO",
        format="%(message)s",
...
async def url(ctx, urls):
    """Download content from Deezer URLs."""
...
async def file(ctx, path):
    """Download content from Deezer URLs listed in a file.
...
@click.option(
    "-f",
    "--first",
    help="Automatically download the first search result.",
    is_flag=True,
)
@click.option(
    "-o",
...
@click.pass_context
@coro
async def search(ctx, first, output_file, num_results, media_type, query):
    """Search Deezer for content.
    ...
    with ctx.obj["config"] as cfg:
        async with Main(cfg) as main:
            if first:
                await main.search_take_first("deezer", media_type, query)
            elif output_file:
                await main.search_output_file(
                    "deezer", media_type, query, output_file, num_results
                )
            else:
                await main.search_interactive("deezer", media_type, query)
            await main.resolve()
            await main.rip()
...
@click.pass_context
@coro
async def id(ctx, media_type, id):
    """Download a Deezer item by ID.

    Example:

        rip id album 12345678
    """
    with ctx.obj["config"] as cfg:
        async with Main(cfg) as main:
            await main.add_by_id("deezer", media_type, id)
            await main.resolve()
            await main.rip()


async def latest_streamrip_version(verify_ssl: bool = True) -> tuple[str, str | None]:
    """Get the latest streamrip version from PyPI."""
